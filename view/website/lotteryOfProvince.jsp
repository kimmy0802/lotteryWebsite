...
<body>
	...
	<!---------------------------------- LOTTERY DETAIL------------------------------------------>
	<main class="col-md-9 mx-1 m-md-auto  col-lg-10 px-md-4" style="min-height: 100vh;">
		<div class="mb-3">
			<h2 class="col-12 pt-3">Danh sách Số trúng thưởng theo Nhà đài</h2>
		</div>
		...
		<c:if test="${pages!=0}">
			...
			<nav class="row justify-content-center"
				...
				<ul id="pagination"
					class="pagination col-12 col-sm-9 justify-content-center">
					<!-- previous pages -->
					<c:if test="${page>1}">
						<li class="page-item">
							<form
					...
					<li class="page-item">
						<form
							...
							<button type="submit" class="btn btn-outline-primary active"
								title="Chuyển đến trang ${page}">${page}</button>
						</form>
					</li>
				...
				<form class="col-12 col-sm-3 d-flex justify-content-center"
					action="${pageContext.request.contextPath}/LotteryOfProvince"
					method="post">
					<input name="action" value="goToPage"  type="hidden" /> 
					<input name="region" value="${region}" type="hidden" />
					...
					<input name="order" value="${order}" type="hidden" /> 
					<select
						name="page" class="form-select form-select-sm p-0 m-0"
						aria-label="Default select">
						<c:forEach var="i" begin="1" end="${pages}" step="1">
							<c:if test='${i==page}'>
								<option class="p-0 m-0" value="${i}" selected="selected">Trang:
									${i}</option>
							</c:if>
							<c:if test='${i!=page}'>
	...
	function changeRDP() {
		...
		$.ajax({
			url : "/LotteryWeb/LotteryOfProvince?action=changeRDP",
