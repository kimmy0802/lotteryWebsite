...
<title>Thêm Nhà đài mới</title>
</head>

<body
	onload='<c:if test="${isError==1}">createFail()</c:if>
<c:if test="${isError==0}">createSuccess("")</c:if>'>

	<!---------------------- navbar---------------------------------->
	<c:import url="/view/website/dashboard.jsp"></c:import>
	<!---------------------------UPDATE------------------------------------------>
	<main class="row col-12 mx-auto col-lg-10 px-md-4 mb-2">
		<div class="col row mx-5 my-2">
			<form class="row col-10 justify-content-center"
				action="${pageContext.request.contextPath}/Provinces" method="post">
				<h2 class="p-0 m-0 text-center">Thêm Nhà đài mới</h2>
				...
				<div class="col-12 col-sm-7  row justify-content-center">
					<input name="action" value="createProvince" type="hidden"/>				
					<div class="col-5">
